...
function windowResized() {
    resizeCanvas(document.documentElement.clientWidth, 400);
    this.preStar();
}

function setup() {
    // キャンバスの設定
    canvas = createCanvas(document.documentElement.clientWidth, 400);
    canvas.parent("canvas")
    canvas.position(8, 210);
...
function draw() {
    ...
    // 花火を打ち上げる間隔を調整
    if (0 === frameCount % 100 && m === 1) {
      // 打ち上がるスピード
      let speed = random(10, 30);
      fireworks.push(new FireWork(random(width), height, 0, speed, 0.98));
    }else if(m === 2 && 0 === frameCount % 100 ){
        let speed = random(10,30);
        pointa();
        fireworks.push(new FireWork(px, py,0, speed, 0.98));
    }
